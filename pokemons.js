...
let hdgfd ="fjghbfjhgf"


let box = document.querySelector(".box");

pokemons.forEach((poke) => {
   let pokimon = document.createElement("div");

   pokimon.classList.add("poke-box");
   pokimon.innerHTML = `
  <p>№ ${poke.id}</p>
  <img src="${poke.img}" alt="" width="100"/>
  <p>${poke.name}</p>
  <div><span>${poke.height} ${poke.weight}</span></div>
  <button class="btn">${poke.type[0]}</button>
  <button class="btn">${poke.type[1]}</button>
  `;
   box.appendChild(pokimon);
});
